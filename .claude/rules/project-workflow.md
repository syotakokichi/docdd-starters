...
| Status | 状態 | コマンド |
|--------|------|---------|
| Backlog | Issue作成済み、未計画 | `/issue` |
| Ready | 計画立案済み、着手待ち | `/plan` |
| In Progress | 実装中 | `/develop` |
| Done | 完了 | `/merge` |
...
|:---:|:---:|:---:|------|
| `/plan`（計画立案） | **実装前** | Pencil → 実装 | 正しいデザインを元に実装したい |
| `/develop`〜`/verify`（実装・検証） | **実装後** | 実装 → Pencil | 実装で判明した改善を Pencil に反映 |

### `/plan` で発見した場合

- **`/plan` の中で Pencil を修正する**（`/develop` に先送りしない）
- 修正手順:
  1. `batch_design` で修正を実施
  2. **確認ゲート**: `get_screenshot` でユーザーに提示し、承認を得る
  3. 修正中に新たな問題が発覚した場合、計画に反映する
- 修正完了後、**正しい Pencil を元に計画を立案**する
- **理由**: 修正を `/develop` に先送りすると、修正中の発見で計画変更が必要になり手戻りが発生する

### `/develop`〜`/verify` で発見した場合

- `/develop` で目視確認時にコード修正 → Pencil も実装に合わせて更新（実装が正）
- `/verify` Step 4 で Pencil と実装の一致を**検証**（修正は `/develop` で完了している前提）
- `/develop` で漏れた修正があれば `/verify` で対応
