...
4. **Close related issues** (triggers Webhook -> Sheets status update)
   ```bash
   # For each issue extracted from "Closes #XXX" in PR body:
   gh issue close [ISSUE_NUMBER]
   ```
   - これにより GitHub Webhook が発火し、Sheets のステータスが「完了」に自動更新される
   - PR 本文の `Closes #XXX` でも自動 Close されるが、明示的に実行して確実に同期
