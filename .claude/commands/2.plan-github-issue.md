...
### Phase 1.5: UI設計（UI変更を伴う場合）

UI変更を伴うIssueの場合は、計画前にデザインを検討:

1. **Pencil.dev でモックアップ作成**を提案
2. デザインが確定したら Issue にモックアップURLを添付
3. コンポーネント分割を計画に含める
4. 詳細は `.claude/skills/design/SKILL.md` を参照
...
**エージェントチーム活用（複雑なIssueの場合）:**
- 複数ドメインにまたがる場合は、並列リサーチエージェントで調査を効率化
- 技術選定では、複数候補を並列で調査し比較検討
- 詳細は `.claude/rules/agent-teams.md` のパターン1・10を参照
...
```bash
gh issue comment $ARGUMENTS --body "## Codex 計画レビュー結果
...
4. 🔴 必須の指摘がある場合:
   - 計画を修正して Issue 本文を再更新
   - 修正後の差分を Issue コメントに追記
